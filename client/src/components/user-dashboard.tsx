...
import { Users, Trophy, Clock, DollarSign, Award, Target } from "lucide-react";
...
export default function UserDashboard() {
  ...
  const CreatedContestCard = ({ contest }: { contest: Contest }) => (
    <Card className="bg-card-bg text-primary border-l-4 border-l-accent" data-testid="created-contest-item">
      <CardContent className="p-6">
        ...
        <div className="flex space-x-3">
          ...
          <Button 
            className="flex-1 bg-gray-200 text-primary hover:bg-gray-300 font-medium" 
            size="sm"
  ...
  const ParticipatedContestCard = ({ contest }: { contest: Contest }) => (
    <Card className="bg-card-bg text-primary border-l-4 border-l-success" data-testid="participated-contest-item">
      <CardContent className="p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href="/create">
              <Button className="bg-accent hover:bg-accent/90 text-accent-foreground font-medium" data-testid="create-new-contest">
                Create New
              </Button>
            </Link>
          </div>
          ...
          {loadingCreated ? (
            <div className="space-y-4">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="bg-card-bg p-6 rounded-lg animate-pulse h-48" data-testid="created-contest-skeleton">
            ...
            <Card className="bg-card-bg text-center py-8" data-testid="no-created-contests">
              <CardContent>
            ...
            <div className="space-y-4" data-testid="created-contests-list">
              {createdContests?.map((contest) => (
                <CreatedContestCard key={contest.id} contest={contest} />
              ))}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          {loadingParticipated ? (
            <div className="space-y-4">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="bg-card-bg p-6 rounded-lg animate-pulse h-48" data-testid="participated-contest-skeleton">
                  <div className="h-4 bg-gray-300 rounded mb-4"></div>
                  <div className="h-6 bg-gray-300 rounded mb-2"></div>
                  <div className="h-16 bg-gray-300 rounded mb-4"></div>
                  <div className="h-10 bg-gray-300 rounded"></div>
            ...
            <Card className="bg-card-bg text-center py-8" data-testid="no-participated-contests">
              <CardContent>
                <div className="flex flex-col items-center">
            ...
            <div className="space-y-4" data-testid="participated-contests-list">
              {participatedContests?.map((contest) => (
                <ParticipatedContestCard key={contest.id} contest={contest} />
              ))}
            </div>
          )}
        </div>
